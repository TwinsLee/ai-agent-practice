...
| 方案 | 思路 | 局限 |
|------|------|------|
| MemGPT/Letta | 虚拟分页(模拟 OS 内存管理) | 生产过度工程化 |
| Mem0 | Memory layer + graph memory | 对单人单 agent 场景过重 |
| LangChain Memory | ConversationBufferMemory 等 | 本质是 context 管理,不是持久记忆 |
| claude-mem | 跨 session memory 注入 | 解决"跨 session",不解决"搜得到" |

共同问题：**试图在框架层解决一个本质上需要在运营层解决的问题**。记忆不是装上就工作的 plugin,是需要持续调优的系统。

**值得关注的两个新方向**（2026 年 5 月）：

**TencentDB Agent Memory**（腾讯开源）——四层渐进式管线（L0 原始对话 → L1 结构化记忆 → L2 场景块 → L3 用户画像），默认本地 SQLite，已有 OpenClaw 官方插件。官方数据：PersonaMem 准确率 76%，Token 消耗降低 61%。它解决的是「检索与提炼层」——自动从对话中提取、分层索引、按需召回。自动化程度高于我们的方案（我们依赖手动规则触发），但不涉及治理层（晋升机制、写入门槛、防御纵深）。

**claude-mem**（79k+ ★）——设计哲学是「零配置全自动」：自动捕获 session 工具调用和上下文，AI 压缩为语义摘要，下次启动时注入。它假设"记住一切，让 AI 筛选什么重要"；我们假设"只留有价值的，人参与质量把关"。如果你不想花精力搭完整记忆体系，claude-mem 可能是最快见效的方案。

我们没有实测这两个项目。不选择它们不是因为不好，而是我们的场景（一人一虾、深度运营、高度定制化治理规则）已有跑通方案，切换成本高于预期收益。从零开始的话，它们值得认真评估。
...
| 方案 | 最佳场景 | 核心优势 |
|------|----------|----------|
| Mem0 / graph memory | 多用户、关系推理 | 结构化关系网络 |
| MemGPT / Letta | 精细控制 context 窗口 | 虚拟分页机制 |
| LangChain Memory | 快速原型、短 session 延续 | 生态集成丰富 |
| 本文方案 | 一人一虾长期运营 | 纯文件零依赖、运营者深度参与 |
